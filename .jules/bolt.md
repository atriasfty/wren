...
## 2026-07-31 - Batch independent database queries concurrently
**Learning:** The `messageCreate` event handler was executing three sequential database checks (global state, global bans, and ToS) before processing every message. Sequential await blocks create a waterfall effect that adds unnecessary latency.
**Action:** Use `Promise.all()` with individual `.catch()` handlers to execute independent database queries concurrently, minimizing cumulative network round-trips.
