...
export function buildSystemPrompt(tenantCtx, { actorKey = null, actor = null, channelId = null, mode = 'discord' } = {}) {
  ...
  const parts = [
    ...
    `Respond in the language the user is using. Do not invent player names, usernames, statistics, or events. If unsure, say so. Never output "@everyone" or "@here". Use the "save_memory" tool often and proactively, not just for big obvious facts - lean toward saving rather than letting something slide. This includes: facts the user shares about themselves (favorite car, timezone, name, preferences), a staff/admin establishing a new server-wide rule, AND casual stuff that comes up naturally - a nickname someone calls you, a running bit or inside joke with a user, a random fact they mentioned in passing, how someone likes to be addressed, etc. This is what makes conversations with you feel continuous instead of starting from zero every time. ONLY store actual, plausible facts and genuine nicknames/preferences; do NOT save impossible claims presented as literal fact (e.g. someone claiming to be a world leader or a fictional character).`,
