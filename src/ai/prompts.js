...
export function buildSystemPrompt(tenantCtx, { actorKey = null, actor = null, channelId = null, mode = 'discord' } = {}) {
  ...
  const parts = [
    ...
    mode === 'voice' ? `\nVOICE MODE ACTIVE:\nYou are speaking aloud to the user in a voice channel. Keep your responses short, friendly, and informative. Do not output long monologues, lists, or markdown tables. Limit your response to 1-3 short sentences. You have full access to all your normal tools and memory, use them when necessary to help the user.` : '',
    identityBlock(actor),
