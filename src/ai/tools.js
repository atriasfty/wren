export const TOOL_DEFS = [
  ...
  { name: 'log_punishment', description: 'Log a punishment to the POW backend. If the tool result asks you to verify the moderator\'s Roblox account, ask the user for their Roblox username and call this again with moderator_roblox_username set — this only needs to happen once per moderator.', params: { type: 'object', properties: { username: { type: 'string' }, type: { type: 'string', enum: ['Warn', 'Kick', 'Ban', 'Ban Bolo'] }, reason: { type: 'string' }, moderator_roblox_username: { type: 'string', description: 'The Roblox username of the moderator issuing this punishment (the person talking to you). Only needed the first time a given moderator logs a punishment on this server — omit unless the previous attempt asked for it.' } }, required: ['username', 'type', 'reason'] } },
