...
export async function createApiServer(client) {
  ...
  // [SECURITY-FIX] Configuration & Headers: Disable X-Powered-By header globally
  ...
  // [SECURITY-FIX] Error Handling: Catch malformed JSON payloads safely without leaking HTML stack trace
  app.use((err, req, res, next) => {
    if (err instanceof SyntaxError && err.status === 400 && 'body' in err) {
      console.error('[api] malformed JSON payload:', err.message);
  ...
  // [SECURITY-FIX] Error Handling: Global catch-all error handler at the route boundary
