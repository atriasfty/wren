...
export async function createApiServer(client) {
  ...
  // [SECURITY-FIX] Configuration & Headers: disable x-powered-by
  ...
  // [SECURITY-FIX] Logic & Error Handling: handle malformed JSON syntax errors securely
  app.use((err, req, res, next) => {
    if (err instanceof SyntaxError && err.status === 400 && 'body' in err) {
      console.error('[api] JSON SyntaxError:', err.message);
  ...
  // [SECURITY-FIX] Logic & Error Handling: catch unhandled exceptions globally
